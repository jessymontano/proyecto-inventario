...
public class ProductoDAO {
    public List<Producto> listProductos() {
        ...
        String sql = "SELECT p.id, p.nombre, p.descripcion, p.cantidad, p.precio, p.almacen_id, a.nombre as almacen_nombre, p.fecha_hora_creacion, p.fecha_hora_ultima_modificacion, p.ultimo_usuario_en_modificar " +
    ...
    public int insertProducto(Producto prod, String usuario) {
        String sql = "INSERT INTO productos(nombre, descripcion, cantidad, precio, almacen_id, fecha_hora_creacion, ultimo_usuario_en_modificar) " +
                "VALUES(?,?,?,?,?,?,?)";
    ...
    public void updateProducto(Producto prod, String usuario) {
        String sql = "UPDATE productos SET nombre=?, descripcion=?, cantidad=?, precio=?, almacen_id=?, fecha_hora_ultima_modificacion=?, ultimo_usuario_en_modificar=? " +
                "WHERE id=?";
        try (Connection c = DatabaseConnection.connect(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, prod.nombre);
            ps.setString(2, prod.descripcion);
